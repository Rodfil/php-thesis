...
<body class="layout-boxed" layout="full-width">
                        ...
                        <div class="col-lg-12 col-md-12 layout-spacing">
                            <div class="statbox widget box box-shadow">
                                ...
                                <?php
                                    ...
                                    $ID = $_GET['id'];
                                    $query = "SELECT * FROM `purpose` WHERE DocumentID = $ID";
                                    if ($result = $mysqli->query($query)) {
                                        while($row = $result->fetch_array()){
                                            echo '<div class="input-group mb-3" dataRow="'.$row['ID'].'">
                                                <input type="text" class="form-control input_Description" RecordID="'.$row['ID'].'" value="'.$row['Purpose_Description'].'">
                                                <button class="btn btn-danger btn_Remove"  dataRow="'.$row['ID'].'" type="button">Remove</button>
                                            </div>';
                                        }
                                    }
                                ?>
    ...
    <script>
                ...
                case "old":
                    $.ajax({
                        ...
                        success: function(evt){
                            if(evt > 0){
                                Message(2,"Purpose successfully deleted");
                                setTimeout(function() { 
                                    location.reload();
                                }, 1000);
                            }
        ...
        $("#btn_Save_Requirement").on("click",function(){
            ...
            var resultArray = [];
            $("#div_Requirement_List").find('.input-group').each(function(){
                var Purpose_Description = $(this).find('.input_Description').val();
            ...
            $.ajax({
                ...
                success: function(evt){
                    if(evt > 0){
                        Message(2,"Purpose successfully saved");
                        setTimeout(function() { 
                            location.reload();
                        }, 1000);
                    }
