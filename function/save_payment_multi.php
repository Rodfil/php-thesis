<?php
    ...
    if ($IsExists == 0){
        $RequestFormIDArr = explode(",",$RequestFormID);
        $AmountArr = explode(",",$Amount);
        $Len = count($RequestFormIDArr);
        for($x = 0; $x < $Len; $x++){

            $ReqID = $RequestFormIDArr[$x];
            $Amount = $AmountArr[$x];

            $query = "INSERT INTO payment(RequestFormID,Amount,PaymentMethod,ReferenceNo)
            VALUES ('$ReqID','$Amount','$PaymentMethod','$ReferenceNo')";
            if ($mysqli->query($query)) {
                ...
                $queryUpdate = "UPDATE request_form SET Status = 2 WHERE ID = $ReqID";
